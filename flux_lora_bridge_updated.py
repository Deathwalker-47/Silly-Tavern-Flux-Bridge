...
2. Together AI ($0.02/image, 15 LoRAs max) - via Together SDK
3. Wavespeed ($0.015/image, 4 LoRAs max) - via REST API

Features:
- Full AUTOMATIC1111 API compatibility
- Keyword-based LoRA injection (case-insensitive substring matching)
- Collects ALL matching LoRAs (not just first match)
- No duplicate LoRAs per generation
...
class Config:
    """Bridge configuration - reads from environment variables with hardcoded fallbacks"""
    HOST = "0.0.0.0"
    PORT = int(os.getenv("BRIDGE_PORT", "7861"))

    # LoRA limits per provider
    MAX_LORAS_DEFAULT = 15
    MAX_LORAS_TOGETHER = 15
    MAX_LORAS_WAVESPEED = 4

    # Paths
    LORA_DICT_PATH = os.getenv("LORA_DICT_PATH", "master_lora_dict.json")

    # Provider credentials (hardcoded fallbacks)
    ...
    TOGETHER_API_KEY = os.getenv("TOGETHER_API_KEY", "tgp_v1_c8bZ8l-5iPB32gQ2UN2ZuoWW_IjvqbrkBuGLPLhiPms")
    ...
    def print_config(cls):
        ...
        logger.info(f"Max LoRAs: Together={cls.MAX_LORAS_TOGETHER}, Wavespeed={cls.MAX_LORAS_WAVESPEED}")
        logger.info("")
        ...
        if cls.HF_SPACE_NAME:
            ...
            logger.info(f"  HF ZeroGPU: ✗ NOT CONFIGURED")
...
#===========================================================================================
# LORA MANAGER - UPDATED WITH NEW MATCHING LOGIC
#===========================================================================================
...
class LoRAManager:
    ...
    def match_loras_by_keywords(self, prompt: str, negative_prompt: str = "") -> List[Dict]:
        """
        Match LoRAs based on keywords - UPDATED LOGIC

        Changes:
        1. Case-insensitive substring matching (keyword.lower() in prompt.lower())
        2. Checks ALL LoRAs, not stopping at first match
        3. No duplicate LoRAs (tracked via seen_ids set)
        """
        matched = []
        seen_ids = set()  # Track seen LoRA IDs to prevent duplicates
        ...
        # Then, check ALL LoRAs for keyword matches
        for lora_id, lora_data in self.lora_dict["loras"].items():
            # Skip if already added (permanent or duplicate)
            if lora_id in seen_ids:
                continue

            # Check if any keyword exists in prompt (case-insensitive substring)
            ...
            for keyword in keywords:
                ...
                if keyword.lower() in combined_text:
                    ...
                    seen_ids.add(lora_id)  # Mark as seen to prevent duplicates
                    break  # Stop checking other keywords for this LoRA once matched

        # Sort by rank (lower = higher priority)
        matched.sort(key=lambda x: x["data"]["rank"])

        logger.info(f"🔍 Matched {len(matched)} LoRAs:")
        for item in matched[:10]:  # Show first 10
    ...
    def build_lora_list(self, matched_loras: List[Dict], max_loras: int) -> List[Dict]:
        """
        Build final LoRA list - UPDATED to respect max_loras limit
        No duplicate checking needed here since match_loras_by_keywords already handles it
        """
        lora_list = []
...
class HFZeroGPUClient(ProviderClient):
    ...
    async def generate(self, prompt: str, negative_prompt: str, loras: List[Dict], params: Dict) -> bytes:
        ...
        # Build LoRA config - YOUR EXACT WORKING FORMAT
        lora_config = json.dumps([
            {
                "repo": "anujithc/Nimya_LoRAs",
                "weights": lora["url"].split("/")[-1],
                "adapter_name": lora["id"][:10],
                "adapter_weight": lora["weight"]
            }
            for lora in loras
        ])

        # Call HF Space - YOUR EXACT WORKING CODE
        result = self.client.predict(
            ...
            params["steps"],
            True,
            params["seed"] if params["seed"] >= 0 else 0,
            params["width"],
            params["height"],
            False,
            "",
...
class TogetherAIClient(ProviderClient):
    """Together AI client - YOUR WORKING CODE"""
    ...
    async def generate(self, prompt: str, negative_prompt: str, loras: List[Dict], params: Dict) -> bytes:
        ...
        lora_urls = [lora["url"] for lora in loras]
        lora_scales = [lora["weight"] for lora in loras]
...
async def txt2img(request: Txt2ImgRequest):
    ...
    # Match LoRAs - UPDATED matching logic
    ...
    logger.info(f"   LoRAs ({len(lora_list)}/{max_loras}):")
