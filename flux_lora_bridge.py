...
# =========================
# 1. Runware (PRIMARY, max 12 LoRAs)
# 2. Wavespeed (fallback, max 4 LoRAs)
# 3. FAL (fallback, max 3 LoRAs)
# 4. Together (fallback, max 2 LoRAs)
...
class Config:
    ...
    def print_config(cls):
        ...
        logger.info(f"  {'✅' if cls.RUNWARE_API_KEY else '❌'} Runware (PRIMARY) - {'CONFIGURED' if cls.RUNWARE_API_KEY else 'NOT SET'}")
        logger.info(f"  {'✅' if cls.WAVESPEED_API_KEY else '❌'} Wavespeed (FALLBACK) - {'CONFIGURED' if cls.WAVESPEED_API_KEY else 'NOT SET'}")
...
async def txt2img(request: Txt2ImgRequest):
    ...
    logger.info(f"📝 [Input] Prompt ({len(request.prompt.split())} words): {request.prompt[:200]}..." if len(request.prompt) > 200 else f"📝 [Input] Prompt: {request.prompt}")
...
# ============================================
# OPENAI-STYLE CHAT COMPLETION PROXY (STUB)
# ============================================
...
def prompt_firewall(text: str) -> str:
    ...
    for pat in INJECTION_PATTERNS:
        if re.search(pat, lowered):
            logger.warning("[ChatProxy] Prompt injection attempt blocked")
            return (
...
# --------------------------------
# STUB ENDPOINTS (no proxy configured)
# --------------------------------

@app.post("/v1/chat/completions")
async def chat_completions(request: Request):
    """Placeholder for future chat proxy integration."""
    logger.info("[ChatProxy] Request received but no proxy backend is configured")
    raise HTTPException(
        status_code=501,
        detail="Chat proxy not configured. No backend is currently active.",
    )
