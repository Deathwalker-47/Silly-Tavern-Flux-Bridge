...
- together (latest with disable_safety_checker support)
...
#===========================================================================================
# CONFIGURATION - Reads from environment variables with hardcoded fallbacks
#===========================================================================================
...
class Config:
    ...
    # Provider credentials from environment (with hardcoded fallbacks)
    ...
    def print_config(cls):
        ...
        if cls.HF_SPACE_NAME:
            ...
            logger.info(f"    Token: {'✓ Set' if cls.HF_TOKEN else '✗ Not set'}")
        else:
...
#===========================================================================================
# PROVIDER CLIENTS - Using your EXACT working code
#===========================================================================================
...
class HFZeroGPUClient(ProviderClient):
    """HuggingFace ZeroGPU client using Gradio - YOUR WORKING CODE"""
    ...
    async def generate(self, prompt: str, negative_prompt: str, loras: List[Dict], params: Dict) -> bytes:
        ...
        # Call HF Space - YOUR EXACT WORKING CODE (positional args)
        result = self.client.predict(
            prompt,              # prompt
            "",                  # image_url
            lora_config,         # lora_strings_json
            0.75,                # image_strength
            params["cfg_scale"], # cfg_scale
            params["steps"],     # steps
            True,                # randomize_seed
            params["seed"] if params["seed"] >= 0 else 0,  # seed
            params["width"],     # width
        ...
        # Read local file and return bytes
        with open(image_path, 'rb') as f:
            return f.read()

class TogetherAIClient(ProviderClient):
    """Together AI client - YOUR WORKING CODE with latest SDK"""
    ...
    async def generate(self, prompt: str, negative_prompt: str, loras: List[Dict], params: Dict) -> bytes:
        ...
        # YOUR EXACT WORKING CODE with disable_safety_checker
        response = self.client.images.generate(
            ...
            steps=params["steps"],
            disable_safety_checker=True,  # ✅ Works with latest together package
            lora_urls=lora_urls,
