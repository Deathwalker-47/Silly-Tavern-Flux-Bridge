...
class HFZeroGPUClient(ProviderClient):
    ...
    async def generate(self, prompt: str, negative_prompt: str, loras: List[Dict], params: Dict) -> bytes:
        ...
        # Build LoRA config JSON - matches your working format
        lora_config = json.dumps([
            {
                ...
                "adapter_name": lora["id"][:10],  # Shorten ID
                "adapter_weight": lora["weight"]
            }
            for lora in loras
        ])

        # Call HF Space - using positional arguments (your working format)
...
class TogetherAIClient(ProviderClient):
    ...
    async def generate(self, prompt: str, negative_prompt: str, loras: List[Dict], params: Dict) -> bytes:
        ...
        # Together AI format - separate arrays (your working format)
        ...
        # Generate image - using your working parameters
        response = self.client.images.generate(
            prompt=full_prompt,
            model="black-forest-labs/FLUX.1-dev",  # ✅ Correct model name
            width=params["width"],
            height=params["height"],
            steps=params["steps"],
            n=1,
            disable_safety_checker=True,  # ✅ Disable safety for NSFW
            lora_urls=lora_urls,
